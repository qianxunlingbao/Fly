                        ...
                        <View style={{ width: '96%', backgroundColor: 'white', marginLeft: '2%', display: this.state.isshow }}>
                            {
                                this.state.musicArray.map((item, index) => {
                                    if (item.music_name.indexOf(this.state.value) !== -1 || item.music_author.indexOf(this.state.value) !== -1) {
                                        return (
                                            <TouchableOpacity style={{ width: '100%', height: 35, flexDirection: 'row', alignItems: 'center', paddingLeft: 15 }} onPress={() => { this.storage(this.state.value) }}>
                                                <Icon name="search1" size={15} color="gray" />
                                                <Text style={{ marginLeft: 10 }}>{item.music_name} {item.music_author}</Text>
                                            </TouchableOpacity>
                                        )
                                    }
                                    if (index == this.state.musicArray.length - 1) {
                                        ...
                                        for (var i = 0; i < this.state.musicArray.length; i++) {
                                            if (this.state.musicArray[i].music_name.indexOf(this.state.value) !== -1 || this.state.musicArray[i].music_author.indexOf(this.state.value) !== -1) {
                                                flag = 0;
                                            }
                                        }
                                        if (flag) {
